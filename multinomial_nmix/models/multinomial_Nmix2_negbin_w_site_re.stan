...
data {
  ...
  vector[R] X_aug; // Habitat covariate (1 = restored or 0 = control)
  vector[R] year_aug; // Year covariate (1 = 2023 or 0 = 2022)
  
  int<lower=0> n_sites; // Number of sites
  int<lower=1> sites_aug[R]; // vector of sites
  
  int<lower=0> n_species; // Number of species
  int<lower=1> species_aug[R]; // vector of species
  
  int<lower=0> M; // Number of zero-augmentated individuals in the population
...
transformed parameters {
  ...
  for (i in 1:R) { // for each site*species combination
    log_mu[i] = // log abundance is equal to..
      ...
      (alpha2_species[species[i]] * years[i]) + // a species specific effect (year)
    ...
    logit_p[i] = // logit (detection probability) is equal to..
        ...
        (beta2 * years[i]) // an effect (year)
      ;
  }
  
  for (i in 1:n_species){
    for (j in 1:n_sites){
      log(lambda[i,j]) = // log abundance is equal to..
        alpha0_species[species_aug[i]] + // a species specific intercept  
        (alpha1_species[species[i]] * X_aug[i]) + // a species specific effect (restoration)
        (alpha2_species[species[i]] * years_aug[i]) + // a species specific effect (year)
        alpha3_site[sites_aug[i]]; // a site specific intercept
        
      cellprobs[i,j] = lambda[i,j] / sum(lambda[i,])
    }
  }
...
model {
  ...
  // year effects
  mu_alpha2 ~ normal(0, 2); // effect of year
  ...
  // year effects
  beta2 ~ normal(0, 2); // effect of year
