...
data {
  ...
  int y[R, 7]; // capture histories matrix (7 observable histories possible)
  int nobs[R]; // total number of individuals observed at each site, i.e. nobs = sum(y[,1:7])
  int K[R]; // some arbitrarilly large maximum site-specific abundance to search up until
...
parameters {
  ...
  real<lower=0> scale_param; // negative binomial distribution scale parameter
  // DETECTION
...
transformed parameters {
  ...
  vector[R] lambda; // site-specific abundance
  ...
  for (i in 1:R) { // for each site*species combination
    lambda[i] = // log abundance is equal to..
